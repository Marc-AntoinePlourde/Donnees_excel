...
def graphique_zoom(donnees, noms, xmin=0, xmax=10, ymin=None, ymax = None):
    ...
    ax.set_xlabel(f"{noms[axe_1]} (mW)")
    ax.set_ylabel(f"{noms[axe_2]} (mW)")
    ...
    axouts = zoomed_inset_axes(ax, 10, loc=4, borderpad=2)
    axouts.plot(don1, don2)
    axouts.set_xlim(95, 105)
    axouts.set_ylim(92, 103)
    plt.xticks(visible=True)
    plt.yticks(visible=True)
    mark_inset(ax, axouts, loc1=2, loc2=3, fc="none", ec="0.5")
    plt.draw()
    plt.show()
